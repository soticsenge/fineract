...
public class OAuth2AuthenticationTest {
    ...
    public void testGetOAuth2UserDetails() throws IOException, InterruptedException {
        ...
        loginAndClaimToken(" https://localhost:8443/fineract-provider/oauth2/authorize" +
                ...
                "&code_challenge_method=S256", requestFormSpec);
        ...
        /*Boolean authenticationCheck = performServerGet(null, responseSpec,
    ...
    public String loginAndClaimToken(String url, RequestSpecification requestSpec) throws IOException, InterruptedException {
    ...
    private String claimTokenOnCallback(RequestSpecification requestSpec, HttpExchange exchange) {
        try (exchange) {
            ...
            String token = performServerPost(
                    requestSpec,
                    responseSpec,
                    "/fineract-provider/oauth2/token",
                    formParams,
                    null);
    ...
    public static <T> T performServerPost(final RequestSpecification requestSpec, final ResponseSpecification responseSpec,
        ...
        final String response  = given().spec(requestSpec).header("Content-Type", "application/x-www-form-urlencoded").formParams(formBody).expect().log().ifError().when().post(putURL).andReturn().asString();
        return (T) JsonPath.from("").get(jsonAttributeToGetBack);
    }
}
